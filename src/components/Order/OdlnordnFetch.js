...
class ClientAsk extends PureComponent {
  ...
  columns = [
    {
      ...
      render: (text, records) => (
        <Link
          target="_blank"
          to={`/sellabout/odlnordnDetail?DocEntry=${text}&ObjType=${records.ObjType}`}
        >
          {text}
        </Link>
      ),
  ...
  componentDidMount() {
    ...
    this.setState(
      ...
      () => {
        this.getOdlnordn(queryData);
      }
    );
  }

  getOdlnordn = async params => {
    const response = await request('/Report/ODLNORDN/ODLNORDN02', {
      method: 'POST',
      data: {
        ...params,
      },
