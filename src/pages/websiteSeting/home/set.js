...
import {Card,Table,Tabs,Button,Popconfirm,message,Divider,Icon,notification,Input} from 'antd'
...
class HomePageSet extends Component {
  ...
  categoryColumns = [
    ...
    },
    {
      ...
      width: 100,
      dataIndex: 'ShowName',
      render:(text,record,index)=><Input value={text} onChange={e=>this.showNameChange(e.target.value,record,index)} />
    },
    
    {
      ...
      render:(text,record)=>text?<img style={{width:100}} alt={record.CategoryName} src={text} />:""
    },
    {
      title: '操作',
      width: 50,
  ...
  addBrandFetch=selectedRows=>{
    ...
    const newArr=selectedRows.map((brand,index)=>{
      ...
      return {
        ...
        BrandName:Name,BrandCode:Code,OrderId:OrderId+index+1
      }
    })
    Object.assign(categorydteail,{TI_Z01905:[...categorydteail.TI_Z01905,...newArr]})
    this.setState({
      categorydteail,brandmodalVisible:false
    })
  ...
  categorySelect=selectedRows=>{
    ...
    if(Level==='2'){
      ...
      selectedRows.map((cateItem,index)=>{
        ...
        if(targetIndex===-1){
          newArr.push({
            Code:categorydteail.Code,ShowName:Name,
            CategoryName:Name,CategoryCode:Code,OrderId:OrderId+index+1
          })
        }
      })
    ...
    if(Level==="3"){
      ...
      selectedRows.map((cateItem,index)=>{
        ...
        if(targetIndex===-1){
          newArr.push({
            Code:categorydteail.Code,ShowName:Name,Father:PCode,
            CategoryName:Name,CategoryCode:Code,OrderId:OrderId+index+1
          })
        }
      })
      Object.assign(categorydteail,{TI_Z01903:[...categorydteail.TI_Z01903,...newArr],thirdCategoryList:[...thirdCategoryList,...newArr]})
    }
    this.setState({
      categorydteail,catemodalVisible:false
    })
    message.success('添加成功，更新后有效')
  ...
  addSKU=selectedRows=>{
    ...
    if(Level==='2'){
      ...
      selectedRows.map((skuItem,index)=>{
        ...
        if(targetIndex===-1){
          newArr.push({
            Code:categorydteail.Code,
            ItemName:Name,SKU:Code,OrderId:OrderId+index+1
          })
        }
      })
    ...
    if(Level==='3'){
      ...
      selectedRows.map((skuItem,index)=>{
        ...
        if(targetIndex===-1){
          newArr.push({
            Code:categorydteail.Code,Father:PCode,
            ItemName:Name,SKU:Code,OrderId:OrderId+index+1
          })
        }
      })
      Object.assign(categorydteail,{TI_Z01904:[...categorydteail.TI_Z01904,...newArr],secondProductList:[...secondProductList,...newArr]})
     
    }
    this.setState({
      categorydteail,skuModalVisible:false
    })
    message.success('添加成功，更新后有效')
  }
  ...
  addThirdCate=PCode=>{
    this.setState({
      PCode,Level:"3",catemodalVisible:true
    })
  }
  ...
  addThirdProduct=PCode=>{
    this.setState({
      PCode,Level:"3",skuModalVisible:true
    })
  }
  ...
  onExpand=(expanded, record)=>{
    const {categorydteail}=this.state
    if(expanded){
     const secondProductList = categorydteail.TI_Z01904.filter((item) => item.Father === record.CategoryCode)
     const thirdCategoryList = categorydteail.TI_Z01903.filter((item) => item.Father === record.CategoryCode)
     this.setState({
      secondProductList,thirdCategoryList,expandedRowKeys:[record.CategoryCode]
     })
    }
  }
      ...
      <Tabs animated={false}>
        <TabPane tab="三级分类" key="category">
          <Table 
            ...
            footer={()=> <Button
              type="primary"
              onClick={() =>this.addThirdCate(record.CategoryCode)}
            >
            添加三级分类
            </Button>}
          />
        ...
        <TabPane tab="产品" key="product">
          <Table 
            ...
            footer={()=> <Button
              type="primary"
              onClick={() =>this.addThirdProduct(record.CategoryCode)}
            >
            添加产品
            </Button>}
          />
  ...
  render() {
    const {categorydteail,brandmodalVisible,PCode,Level,catemodalVisible,skuModalVisible,uploadmodalVisible,expandedRowKeys}=this.state
    ...
    return (
      <Card bordered={false}>
        ...
        <Tabs onChange={this.tabChange}>
          <TabPane tab="二级分类" key="TI_Z01902">
            <Table 
              ...
              footer={()=> <Button
                type="primary"
                onClick={() => this.setState({ catemodalVisible: true })}
              >
              添加二级分类
              </Button>}
            />
          ...
          <TabPane tab="排行榜物料" key="TI_Z01906">
            <Table 
              ...
              footer={()=><div>
                <Button
                type="primary"
                onClick={() => this.setState({ skuModalVisible: true })}
              >
              添加物料
              </Button>(最多添加8个)
              </div> }
        ...
        <SKUModal Category={PCode} Type="checkbox" {...skuParentMethods} modalVisible={skuModalVisible} />
        <OrderAttachUpload {...uploadmodalMethods} modalVisible={uploadmodalVisible} />
        <CateModal PCode={PCode} Level={Level} Type="checkbox" handleSubmit={this.categorySelect} modalVisible={catemodalVisible} handleModalVisible={this.handleModalVisible} />
        <FooterToolbar>
